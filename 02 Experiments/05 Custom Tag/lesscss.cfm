<cfif thistag.ExecutionMode EQ "start"><cfsilent>
	...
	<cfset lessFilesContent = "" />
	<cfloop list="#attributes.input#" index="i">
		<cffile action="read" file="#expandPath(i)#" variable="currentFileContent" />
		<cfset lessFilesContent = lessFilesContent & currentFileContent />
	</cfloop>

	<cfset css = runJS(getLessJS(), "compileString", [lessFilesContent]) />

	<cffile action="write" file="#expandPath(attributes.output)#" addnewline="No" output="#css#" />

	<cfscript>
		function runJS(jsString, methodName, args=[]) {
			var context = createObject("java", "org.mozilla.javascript.ContextFactory").getGlobal().enterContext();
			try {
				var scope = context.initStandardObjects(); // org.mozilla.javascript.ScriptableObject
				context.evaluateString(scope, toString(arguments.jsString), "inlineScript", 1, javaCast('null', '') );
				try {
					var output = scope.get(arguments.methodName, scope).call(context, scope, scope, arguments.args).toString();
				} catch (Any e) {
					writeOutput( "LESS Parsing Error: " & e.value.get("message",scope) );
					abort;
				}
			} finally {
				context.exit();
...
<cffunction name="getLessJS">
	<cfset var lessjs = "" />
	<cfsavecontent variable="lessjs">
// =============================================================================
// Asual JS libs to allow LESS to run in Rhino
// Asual.browser.js
// =============================================================================
var timers = [],
    window = {
...
// =============================================================================
// Finally, the Asual.js engine to simplify LESS calls
// Asual.engine.js
// =============================================================================
...
</cfsilent><cfelse><cfoutput><link type="text/css" rel="Stylesheet" href="#attributes.output#" /></cfoutput></cfif>